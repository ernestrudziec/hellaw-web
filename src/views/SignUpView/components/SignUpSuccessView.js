...
import BeforeUserAuthSpinner
    from "../../../components/LoadingSpinners/BeforeUserAuthSpinner/BeforeUserAuthSpinner";
...
background: url("https://images.pexels.com/photos/48148/document-agreement-documents-sign-48148.jpeg?auto=compress&cs=tinysrgb&h=650&w=940") center no-repeat;
    ...
    if (isUserSignUpPosted) {
        ...
        return (
            <StyledSignUpSuccessView>
                <Logo white/>
                <h1>
                Pomyślnie utworzono konto, możesz się
                zalogować już teraz!
                </h1>
                <h3>Twój adres email:</h3>
                <h2>{Cookie.get('email')}</h2>
                <h4>To jednocześnie twój login, dzięki któremu
                możesz logować się do HELLaw.
                </h4>
                <Link to='/login'><StyledButton login> Zaloguj się teraz</StyledButton></Link>
            </StyledSignUpSuccessView>
        );
    } else {
        return(
            <BeforeUserAuthSpinner/>
        );
    }
